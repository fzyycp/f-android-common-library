...
public class UrlUtils {
            ...
            if (!CollectionsUtils.isEmpty(splitHost)) {
                for (int i = 0; i < splitHost.length; i++) {
                    ...
                    if (!isEncoded(hostSegment, charset)) {
                        ...
                        // check specials
                        if (!CollectionsUtils.isEmpty(specials)) {
    ...
    private static String getEncodedFile(String path, String query, String ref, String charset, EncodeInfo[] specials) {
        ...
        try {
            ...
            // included '/'
            if (!CollectionsUtils.isEmpty(splitPath)) {
            ...
            if (!TextUtils.isEmpty(query)) {
                ...
                // included '&'
                if (!CollectionsUtils.isEmpty(splitQuery)) {
                    for (int i = 0; i < splitQuery.length; i++) {
                        ...
                        // included '='
                        if (!CollectionsUtils.isEmpty(keyValue) && keyValue.length >= 2) {
